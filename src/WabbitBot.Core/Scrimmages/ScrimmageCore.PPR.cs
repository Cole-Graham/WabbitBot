...
namespace WabbitBot.Core.Scrimmages
{
    public partial class ScrimmageCore
    {
        ...
        /// </summary>
        public async Task<CreateProvenPotentialRecordResponse> HandleCreateProvenPotentialRecordRequest(
            CreateProvenPotentialRecordRequest request)
        {
            ...
            if (string.Equals(match.ParentType, "Scrimmage", StringComparison.OrdinalIgnoreCase) && match.ParentId.HasValue)
            {
            ...
            double challengerRating = scrimTeam1Rating ?? (team1.Stats.ContainsKey(teamSize) ? team1.Stats[teamSize].CurrentRating : 0.0);
            double opponentRating = scrimTeam2Rating ?? (team2.Stats.ContainsKey(teamSize) ? team2.Stats[teamSize].CurrentRating : 0.0);
            ...
            try
            {
                ...
                foreach (var teamId in teamsWithRecords ?? Enumerable.Empty<Guid>())
                {
                    ...
                    if (team != null)
                    {
                        ...
                        if (team.Stats.TryGetValue(TeamSize.TwoVTwo, out var stats))
                        {
                            currentRating = stats.CurrentRating;
                        }
                        else if (team.Stats.Any())
                        {
                            currentRating = team.Stats.First().Value.CurrentRating;
                        }
                    }
