...
namespace WabbitBot.Core.Common.Models
{
    public partial class PlayerCore : IPlayerCore
    {
        ...
        public static class Validation
        {
            ...
            {
                return !(player?.TeamIds?.Count > 0) && !(player?.IsArchived ?? true);
            }
        ...
        public async Task<Result> ArchiveAsync(Guid playerId)
        {
            ...
            player.ArchivedAt = DateTime.UtcNow;
            var update = await CoreService.Players.UpdateAsync(player, DatabaseComponent.Repository);
            return update.Success ? Result.CreateSuccess() : Result.Failure(update.ErrorMessage ?? "Failed to archive player");
        }
        ...
        public async Task<Result> UnarchiveAsync(Guid playerId)
        {
            ...
            var update = await CoreService.Players.UpdateAsync(player, DatabaseComponent.Repository);
            return update.Success ? Result.CreateSuccess() : Result.Failure(update.ErrorMessage ?? "Failed to unarchive player");
        }
