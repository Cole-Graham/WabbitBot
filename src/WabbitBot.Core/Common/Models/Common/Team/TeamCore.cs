...
namespace WabbitBot.Core.Common.Models
{
    public partial class TeamCore : ITeamCore
    {
        ...
        public async Task RemovePlayer(Guid teamId, Guid playerId)
        {
            ...
            var member = team.Roster.FirstOrDefault(m => m.PlayerId == playerId);
            if (member != null)
            {
        ...
        public async Task ChangeCaptain(Guid teamId, Guid newCaptainId)
        {
            ...
            var currentCaptain = team.Roster.FirstOrDefault(m => m.Role == TeamRole.Captain);
            ...
            // Find new captain
            var newCaptain = team.Roster.FirstOrDefault(m => m.PlayerId == newCaptainId);
            if (newCaptain != null)
            {
                // Promote new captain
        ...
        public async Task<Result> UpdateStats(Guid teamId, TeamSize teamSize, bool isWin)
        {
            var teamResult = await CoreService.Teams.GetByIdAsync(teamId,
                DatabaseComponent.Repository);
            if (!teamResult.Success)
            ...
            if (!team.Stats.TryGetValue(teamSize, out var stats))
            {
                stats = new Models.Stats { TeamId = teamId, TeamSize = teamSize, };
                team.Stats.Add(teamSize, stats);
            }
            ...
            var updateTeamResult = await CoreService.Teams.UpdateAsync(team,
                DatabaseComponent.Repository);
            var updateStatsResult = await CoreService.Stats.UpdateAsync(stats,
        ...
        // Moved from StatsCore
        public async Task<Result> UpdateRating(Guid teamId, TeamSize teamSize, double newRating)
        {
            var teamResult = await CoreService.Teams.GetByIdAsync(teamId,
                DatabaseComponent.Repository);
            if (!teamResult.Success)
            ...
            {
                return Result.Failure(
                    $"Failed to update rating for team {teamId}: " +
                    $"{updateTeamResult.ErrorMessage} / " +
                    $"{updateStatsResult.ErrorMessage}"
                );
            }
        ...
        public static class Accessors
        {
            ...
            {
                return team.Roster.Where(m => m.IsActive && m.IsTeamManager).Select(m => m.PlayerId).ToList();
            }

            public static List<TeamMember> GetActiveMembers(Team team)
            {
                return team.Roster.Where(m => m.IsActive).ToList();
            }

            public static TeamMember? GetMember(Team team, Guid playerId)
            {
                return team.Roster.FirstOrDefault(m => m.PlayerId == playerId);
            }

            public static bool HasPlayer(Team team, Guid playerId)
            {
                return team.Roster.Any(m => m.PlayerId == playerId);
            }

            public static bool HasActivePlayer(Team team, Guid playerId)
            {
                return team.Roster.Any(m => m.PlayerId == playerId && m.IsActive);
            }

            public static int GetActivePlayerCount(Team team)
            {
                return team.Roster.Count(m => m.IsActive);
            }

            public static List<Guid> GetActivePlayerIds(Team team)
            {
                return team.Roster.Where(m => m.IsActive).Select(m => m.PlayerId).ToList();
            }

            public static List<Guid> GetCorePlayerIds(Team team)
            {
                return team.Roster.Where(m => m.IsActive && (m.Role == TeamRole.Core || m.Role == TeamRole.Captain)).Select(m => m.PlayerId).ToList();
            }

            public static List<Guid> GetCaptainIds(Team team)
            {
                return team.Roster.Where(m => m.IsActive && m.Role == TeamRole.Captain).Select(m => m.PlayerId).ToList();
            }

            public static bool IsCaptain(Team team, Guid playerId)
            {
                return team.Roster.Any(m => m.PlayerId == playerId && m.IsActive && m.Role == TeamRole.Captain);
            }

            public static bool IsTeamManager(Team team, Guid playerId)
            {
                return team.Roster.Any(m => m.PlayerId == playerId && m.IsActive && m.IsTeamManager);
            }
