...
namespace WabbitBot.Core.Common.Models
{
    public partial class TeamCore
    {
        public class Stats
        {
            public static double GetWinRate(Models.Stats stats)
            {
                var totalMatches = stats.Wins + stats.Losses;
                return totalMatches == 0 ? 0 : (double)stats.Wins / totalMatches;
            }

            public static int GetTotalMatches(Models.Stats stats)
            {
            ...
            public static double GetEffectiveRating(Models.Stats stats, TeamVarietyStats varietyStats)
            {
