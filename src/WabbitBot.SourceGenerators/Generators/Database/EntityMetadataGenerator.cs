    ...
    public class EntityMetadataGenerator : IIncrementalGenerator
    {
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            try
            {
                context.RegisterPostInitializationOutput(ctx =>
                    ctx.AddSource(
                        "__WG_Init_EntityMetadataGenerator.g.cs",
                        "// EntityMetadataGenerator: Initialize hit"
                    )
                );
            }
            catch (Exception ex)
            {
                context.RegisterPostInitializationOutput(ctx =>
                    ctx.AddSource(
                        "__WG_Init_EntityMetadataGenerator_Error.g.cs",
                    ...
                    transform: static (ctx, _) =>
                    {
                        ...
                        var attr = classSymbol
                            ...
                            .FirstOrDefault(a =>
                                ...
                                == WabbitBot.Generator.Shared.AttributeNames.EntityMetadata
                            );
            ...
            var entityConfigSource = entityInfos.Select(
                (entities, ct) =>
                {
                    ct.ThrowIfCancellationRequested();
                    return GenerateEntityMetadataConfiguration(
                        entities.OfType<WabbitBot.Generator.Shared.Metadata.EntityMetadataInfo>()
                    );
                }
            );

            context.RegisterSourceOutput(
                entityConfigSource.Combine(isCoreProject),
                (spc, tuple) =>
                {
                    if (!tuple.Right)
                        return;
            ...
            var dbConfigsSource = entityInfos.Select(
                (entities, ct) =>
                {
                    ct.ThrowIfCancellationRequested();
            ...
            var factorySource = entityInfos.Select(
                (entities, ct) =>
                {
                    ct.ThrowIfCancellationRequested();
                    var nonNull = entities.OfType<WabbitBot.Generator.Shared.Metadata.EntityMetadataInfo>();
            ...
            context.RegisterSourceOutput(
                dbConfigsSource.Combine(isCoreProject),
                (spc, tuple) =>
                {
                    if (!tuple.Right)
                        return;
            ...
            context.RegisterSourceOutput(
                factorySource.Combine(isCoreProject),
                (spc, tuple) =>
                {
                    if (!tuple.Right)
                        return;
        ...
        )
        {
            ...
            sourceBuilder.AppendLine(
                CreateFileHeader("EntityMetadataGenerator", "Auto-generated entity metadata configuration")
            );
            ...
            {
                sourceBuilder.AppendLine(
                    CommonTemplates.CreateGeneratedDoc($"Metadata for {metadata.ClassName} entity.")
                );
                sourceBuilder.AppendLine(
                    $"        public const string {metadata.ClassName}TableName = \"{metadata.TableName}\";"
                );
                sourceBuilder.AppendLine(
                    $"        public const string {metadata.ClassName}ArchiveTableName = \"{metadata.ArchiveTableName}\";"
                );
                sourceBuilder.AppendLine(
                    $"        public static readonly string[] {metadata.ClassName}Columns = new[]\n"
                        + $"            {{ {string.Join(", ", metadata.SnakeCaseColumnNames.Select(c => $"\"{c}\""))} }};"
                );
                sourceBuilder.AppendLine();
            }
        ...
        )
        {
            ...
            foreach (var m in entityMetadata)
            {
                ...
                sb.AppendLine($"    /// </summary>");
                sb.AppendLine(
                    $"    public sealed class {m.ClassName}DbConfig : EntityConfig<{m.EntityType.ToDisplayString()}>, IEntityConfig"
                );
                sb.AppendLine("    {");
                ...
                sb.AppendLine(
                    $"            columns: new[] {{ {string.Join(", ", m.SnakeCaseColumnNames.Select(c => $"\"{c}\""))} }},"
                );
        ...
        )
        {
            ...
            foreach (var m in entityMetadata)
            {
                ...
                sb.AppendLine(
                    $"        private static readonly Lazy<{configClassName}> {lazyField} = new(() => new {configClassName}());"
                );
