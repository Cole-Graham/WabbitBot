...
namespace WabbitBot.DiscBot.App
{
    ...
    public static class DiscBotBootstrap
    {
        ...
        public static async Task StartDiscordClientAsync(IBotConfigurationService config)
        {
            ...
            try
            {
                ...
                _client = DiscordClientBuilder
                    ...
                    .ConfigureEventHandlers(b =>
                    {
                        ...
                        b.HandleSocketClosed((client, args) =>
                        {
                            var error = new Exception($"Socket closed with code {args.CloseCode}: {args.CloseMessage}");
                            return DiscBotService.ErrorHandler.CaptureAsync(error, "Discord socket closed", nameof(DiscBotBootstrap));
                        });

                        // Handle zombied connection
                        b.HandleZombied((client, args) =>
                        {
                            var error = new Exception("Discord connection zombied");
                            return DiscBotService.ErrorHandler.CaptureAsync(error, "Discord connection zombied", nameof(DiscBotBootstrap));
                        });

                        // Handle component interactions (buttons, selects, etc.)
                        b.HandleComponentInteractionCreated(async (client, args) =>
                        {
                            await HandleComponentInteractionAsync(client, args);
                        });

                        // Handle modal submissions
                        b.HandleModalSubmitted(async (client, args) =>
                        {
            ...
            {
                await DiscBotService.ErrorHandler.CaptureAsync(ex, "Failed to initialize Discord client", nameof(StartDiscordClientAsync));
                throw;
            }
        }
        ...
        /// </summary>
        private static async Task HandleComponentInteractionAsync(DiscordClient client, global::DSharpPlus.EventArgs.ComponentInteractionCreatedEventArgs args)
        {
            try
            {
            ...
            {
                await DiscBotService.ErrorHandler.CaptureAsync(ex, $"Failed to handle component interaction: {args.Interaction.Data.CustomId}", nameof(HandleComponentInteractionAsync));
            }
        }
        ...
        /// </summary>
        private static async Task HandleModalSubmitAsync(DiscordClient client, global::DSharpPlus.EventArgs.ModalSubmittedEventArgs args)
        {
            try
            {
                var customId = args.Interaction.Data.CustomId;
            ...
            {
                await DiscBotService.ErrorHandler.CaptureAsync(ex, $"Failed to handle modal submission: {args.Interaction.Data.CustomId}", nameof(HandleModalSubmitAsync));
            }
        }
        ...
        public static async Task StopAsync()
        {
            if (_client is not null)
            {
                ...
                {
                    await DiscBotService.ErrorHandler.CaptureAsync(ex, "Failed to stop Discord client", nameof(StopAsync));
                }
            }
        }
    }
