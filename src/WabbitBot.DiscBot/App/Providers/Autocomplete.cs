...
namespace WabbitBot.DiscBot.App.Providers
{
    ...
    public sealed class UserTeamAutoComplete : IAutoCompleteProvider
    {
            ...
            AutoCompleteContext context)
        {
            ...
            var teamSizeEnum = teamSize != null
    ...
    public sealed class OpponentTeamAutoComplete : IAutoCompleteProvider
    {
            ...
            AutoCompleteContext context)
        {
            ...
            var teams = await CoreService.WithDbContext(async db =>
            {
                ...
                var player = await db.MashinaUsers
                    .Where(mu => mu.DiscordUserId == context.User.Id)
    ...
    public sealed class RosterPlayerAutoComplete : IAutoCompleteProvider
    {
        ...
        {
            var term = context.UserInput ?? string.Empty;
            var challengerTeamName = context.Options.FirstOrDefault(o => o.Name == "challengerTeamName")?.Value?.ToString();
            var teamSize = context.Options.FirstOrDefault(o => o.Name == "teamSize")?.Value?.ToString();
            ...
            {
                return await db.Teams
                    .Where(t => t.Name.Equals(challengerTeamName, StringComparison.OrdinalIgnoreCase))
            ...
            return players
                .Select(p => new DiscordAutoCompleteChoice(p.DisplayName, p.PlayerId.ToString()));
        }
    }
    ...
    public sealed class ActiveChallengeAutoComplete : IAutoCompleteProvider
    {
            ...
            AutoCompleteContext context)
        {
            ...
            {
                return await db.ScrimmageChallenges
                    .Where(c => c.ChallengeStatus == ScrimmageChallengeStatus.Pending)
                    .Include(c => c.ChallengerTeam)
                    .Include(c => c.OpponentTeam)
                    .Where(c => c.ChallengerTeam != null && c.OpponentTeam != null)
                    .Where(c => c.ChallengerTeam!.Name.Contains(term, StringComparison.OrdinalIgnoreCase) ||
                               c.OpponentTeam!.Name.Contains(term, StringComparison.OrdinalIgnoreCase))
                    .OrderByDescending(c => c.CreatedAt)
                    .Select(c => new
                    {
                        DisplayName = $"{c.ChallengerTeam!.Name} vs {c.OpponentTeam!.Name}",
                        c.Id
                    })
                    .Take(25)
                    .ToListAsync();
            ...
            return challenges
                .Select(c => new DiscordAutoCompleteChoice(c.DisplayName, c.Id.ToString()));
        }
    }
}