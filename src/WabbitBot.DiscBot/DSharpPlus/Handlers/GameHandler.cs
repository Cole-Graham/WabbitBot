...
namespace WabbitBot.DiscBot.DSharpPlus.Handlers
{
    ...
    public static class GameHandler
    {
        ...
        public static void Initialize()
        {
            ...
            DiscBotService.EventBus.Subscribe<DeckDmStartRequested>(HandleDeckDmStartRequestedAsync);
            DiscBotService.EventBus.Subscribe<DeckDmUpdateRequested>(HandleDeckDmUpdateRequestedAsync);
            DiscBotService.EventBus.Subscribe<DeckDmConfirmRequested>(HandleDeckDmConfirmRequestedAsync);
        }
        /// <summary>
        ...
        private static async Task<Result> HandleGameContainerRequestedAsync(GameContainerRequested evt)
        {
            try
            {
                ...
                // For now, this is a placeholder - the thread ID would be stored when the match is provisioned
                // When state tracking is implemented, uncomment the following:
                // var threadId = await MatchStateTracker.GetThreadIdAsync(evt.MatchId);
                // var thread = await client.GetChannelAsync(threadId);
                // if (thread is null)
                // {
                //     return Result.Failure("Thread not found");
                // }
                // return await Renderers.GameRenderer.RenderGameContainerAsync(client, thread, evt.MatchId, evt.GameNumber, evt.ChosenMap);

                await DiscBotService.ErrorHandler.CaptureAsync(
                    new NotImplementedException("Game container rendering blocked by missing state tracker"),
                    $"Game container requested for match {evt.MatchId}, game {evt.GameNumber} on map {evt.ChosenMap}",
                    nameof(HandleGameContainerRequestedAsync));

                return Result.CreateSuccess("Game container placeholder (state tracking not yet implemented)");
            }
            catch (Exception ex)
            {
                await DiscBotService.ErrorHandler.CaptureAsync(
        ...
        private static async Task<Result> HandleDeckDmStartRequestedAsync(DeckDmStartRequested evt)
        {
            ...
            catch (Exception ex)
            {
                await DiscBotService.ErrorHandler.CaptureAsync(
                    ex,
                    $"Failed to handle deck DM start request for match {evt.MatchId}, game {evt.GameNumber} to player {evt.PlayerDiscordId}",
                    nameof(HandleDeckDmStartRequestedAsync));
