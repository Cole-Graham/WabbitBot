...
// /// <summary>
// /// Choice provider for game sizes - automatically generated from TeamSize enum
// /// </summary>
// public class TeamSizeChoiceProvider : IChoiceProvider
// {
//     public async ValueTask<IEnumerable<DiscordApplicationCommandOptionChoice>> ProvideAsync(CommandParameter parameter)
//     {
//         return await Task.FromResult(Enum.GetValues<TeamSize>()
...
//     private static string GetDisplayName(TeamSize TeamSize)
//     {
...
// /// </summary>
// public class TeamTeamSizeChoiceProvider : IChoiceProvider
// {
//     public async ValueTask<IEnumerable<DiscordApplicationCommandOptionChoice>> ProvideAsync(CommandParameter parameter)
//     {
//         return await Task.FromResult(Enum.GetValues<TeamSize>()
//             .Where(size => size != TeamSize.OneVOne) // Exclude 1v1 for team commands
//             .Select(size => new DiscordApplicationCommandOptionChoice(
//                 GetDisplayName(size),
//                 GetDisplayName(size)
//             )));
...
//         var TeamSizeParameter = ctx.Arguments.Keys.FirstOrDefault(p => p.Name == "size");
//         if (TeamSizeParameter != null &&
//             ctx.Arguments.TryGetValue(TeamSizeParameter, out var TeamSizeObj) &&
//             TeamSizeObj != null)
//         {
//             var TeamSizeString = TeamSizeObj?.ToString();
//             if (!string.IsNullOrEmpty(TeamSizeString) && Game.Validation.TryParseTeamSize(TeamSizeString, out var TeamSize))
//             {
//                 // Filter teams by the selected game size, excluding 1v1 teams
//                 var teams = await teamService.SearchTeamsByTeamSizeAsync(
//                     userInput, TeamSize, 25);

//                 return teams.Where(team => team.TeamSize != TeamSize.OneVOne)
...
//         return allTeams.Where(team => team.TeamSize != TeamSize.OneVOne)
//             .Select(team => new DiscordAutoCompleteChoice(
//                 GetDisplayName(team),
//                 team.Name
//             ));
...
//         var TeamSizeDisplay = Game.Helpers.GetTeamSizeDisplay(team.TeamSize);
//         return $"{baseName} ({TeamSizeDisplay})";
//     }
// }
...
//                 team.TeamSize != TeamSize.OneVOne &&
